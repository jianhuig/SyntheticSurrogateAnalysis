...
# read height data
dat <- readRDS('field_50_cleaned.rds')
  
weight <- read.table("height.tab", header = TRUE) %>% select(f.eid, f.21002.0.0)
dat <- dat %>% left_join(weight)
...
# remove NA heights
dat <- dat %>% filter(!is.na(f.50.0.0))
# upper and lower 10%
upper <- quantile(dat$f.50.0.0, 0.9, na.rm = TRUE)
lower <- quantile(dat$f.50.0.0, 0.1, na.rm = TRUE)
dat <- dat %>% mutate(ind = between(f.50.0.0, lower, upper))
...
test <- dat %>% inner_join(test_id)  %>% mutate(age = f.21022.0.0, sex = f.22001.0.0,
                                                weight = f.21002.0.0, waist = f.48.0.0, 
                                                height = f.50.0.0) %>% select(f.eid, height, age, sex, weight, waist, int, starts_with("PC"), ind) %>%
  rename(oracle_int = int) %>%
  na.omit()

# Random Forest Model
train <- train %>% mutate(age = f.21022.0.0, sex = f.22001.0.0,
                          weight = f.21002.0.0, waist = f.48.0.0, 
                          height = f.50.0.0) %>%
  filter(ind) %>%
  select(height, age, sex, weight, waist) %>% 
  na.omit() 
...
yhat <- predict(model.rf, 
                data = test %>%
                  select(age, sex, weight, waist))$predictions
test$yhat <- yhat

# Set NAs
#test$int <- test$height
test$int <- test$oracle_int
...
missing_index <- sample(1:length(yhat), size = floor(length(yhat)*0.5))
test[missing_index,"int"] <- NA
# Inverse Normal Transformation
n <- nrow(test)
...
saveRDS(results, file = paste0("height_remove_top10bottom10_try.rds"))