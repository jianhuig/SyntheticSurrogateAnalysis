...
PlotSim <- function(sim) {
  ...
  df$miss <- factor(
    df$miss,
    levels = c(-0.01, 0, 0.25, 0.5, 0.75, 0.90),
    ordered = TRUE
  )
  ...
  q <- ggplot(data = df) + 
    ...
    geom_boxplot(
      aes(
        x = method,
        y = bias,
        ymin = quantile(bias, 0.05),
        lower = quantile(bias, 0.25),
        middle = quantile(bias, 0.50),
        upper = quantile(bias, 0.75),
        ymax = quantile(bias, 0.95),
        fill = miss
      )
    ) +
    scale_fill_brewer(
      name = "Missingness",
      palette = "Blues",
      labels = c("None", "0%", "25%", "50%", "75%", "90%")
    ) + 
    scale_x_discrete(
      name = "Method",
      labels = c("Standard", rep("SynSurr", times = 5))
    ) + 
    scale_y_continuous(
      name = "Bias"
    )
...
# Simulations.
set.seed(10101)
sim_1 <- Sim(rho = 0.0, reps = 5000)
sim_1_marg <- sim_1 %>% dplyr::filter(method == "obs")

set.seed(10101)
sim_2 <- Sim(rho = 0.75, reps = 5000)
sim_2_marg <- sim_2 %>% dplyr::filter(method == "obs")

# Harmonize.
sim_1 <- rbind(sim_1, sim_2_marg)
sim_2 <- rbind(sim_2, sim_1_marg)

# Case 1: Rho = 0.0.
q0 <- PlotSim(sim_1) + 
  ggtitle(expression(Correlation~rho==0.00)) +
  ylim(-0.12, 0.12) + ylab("Bias")

legend <- ggpubr::get_legend(q0)
legend <- ggpubr::as_ggplot(legend)
q0 <- q0 + theme(legend.position = "none")

# Case 2: Rho = 0.75
q1 <- PlotSim(sim_2) + 
  ggtitle(expression(Correlation~rho==0.75)) +
...
# Setting: rho = 0.
tab1 <- TabSim(sim_1) %>%
  dplyr::filter((method == "obs" & rho == 0) | method == "ss")
tab1$re <- tab1$se[1]^2 / tab1$se^2 
tab1$miss[1] <- NA

# Setting: rho = 0.75.
tab2 <- TabSim(sim_2) %>%
  dplyr::filter((method == "obs" & rho == 0) | method == "ss")
tab2$re <- tab2$se[1]^2 /  tab2$se^2
tab2$miss[1] <- NA

# Overall table.
tab <- rbind(tab1, tab2)
print(xtable(tab, digits = 3), include.rownames = FALSE)
