...
cov_column <- colnames(height %>% dplyr::select(-paste0("f.", field, ".0.0")))[grep(".0.0$", colnames(height %>% dplyr::select(-paste0("f.", field, ".0.0"))))]
# remove ethnicity
...
test_imputed_linear <- mice(
    data = rbind(train %>% select(!!paste0("f.", field, ".0.0"), f.21022.0.0, f.22001.0.0), 
...
write.table(test %>%
    ...
    rename(`#FID` = f.eid), "Data/height_imputed.txt", sep = "\t", row.names = F, quote = F)
...
write.table(test %>%
    ...
    rename(`#FID` = f.eid), "Data/height_covariate.txt", sep = "\t", row.names = F, quote = F)

# write imputed data to Run SynSurr
saveRDS(test %>%
select(int, imputed_rf_1, imputed_linear_1, imputed_rf_permute_1, imputed_rf_negate_1, f.22001.0.0, f.21022.0.0, starts_with("PC")), "Data/height_imputed.rds")
