...
#' @return Data.frame.
GenSingleImp <- function(data, imp_param) {
...
Sim <- function(n = 1e3, reps = 1e3) {
  ...
  results <- lapply(seq_len(reps), function(i) {
    ...
    train_data <- DGP(n)
    imp_param <- FitImpModel(train_data)
