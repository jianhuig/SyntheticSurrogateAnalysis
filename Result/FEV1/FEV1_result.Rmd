...
temp %>%
  inner_join(yhat) %>%
  ggplot(aes(x = f.20150.0.0, y = yhat)) +
  geom_point() +
...
avg_chisq <- avg_chisq %>%
  data.frame() %>%
  mutate(missing = c(0, 0.25, 0.5, 0.75))
...
avg_chisq %>% ggplot(aes(x = c(0, 0.25, 0.5, 0.75), y = bivariate / marginal)) +
  geom_point() +
  geom_line() +
