...
result <- readRDS("/Users/jianhuigao/Desktop/SyntheticSurrogateAnalysis/Data/power1e4.rds")
...
t1e_power_table <- inner_join(t1e_table, power_table, by = c("missing",  
                                                             "rho"))
t1e_power_table[, c('missing')] <- round(100*t1e_power_table[, c('missing')],1)

colnames(t1e_power_table) <- c("Missing Rate (\\%)", "$\\rho$",  "Type I Error",
                               "$\\chi^2$",   "Power")  #,  "$\\chi^2$")
print(xtable(t1e_power_table,
             digits=c(0, 0, rep(2, ncol(t1e_power_table) - 1))), 
...
re_h2_plot <- re %>% ggplot(aes(x = h2*100, y = re, group = rho, color = factor(rho))) +
  geom_point() +
  geom_line() +
  facet_wrap(. ~ missing, labeller = as_labeller(missing_names)) +
  scale_color_manual(name = expression(rho), values = biv_palette) +
