...
## Total Body Mass Prediction

In total, 25697 individuals were removed in the association analysis due to being first-degree relatives with other people. After excluding people with missing total mass measured by DXA, 2109 remaining individuals served as training set. The covariates are age, sex, height, weight and BMI, where weight and BMI are impedance measures. 

```{r}
temp <- read.table(file = "/Users/jianhuigao/Library/CloudStorage/OneDrive-UniversityofToronto/EHR_research/SyntheticSurrogateAnalysis/Data/TotalMass.tab", header = TRUE)
yhat <- readRDS("/Users/jianhuigao/Library/CloudStorage/OneDrive-UniversityofToronto/EHR_research/SyntheticSurrogateAnalysis/Data/predicted_23283.rds")
temp %>% inner_join(yhat) %>% ggplot(aes(x = f.23283.0.0/1000, y = yhat/1000)) + geom_point() + geom_abline(slope = 1, col = 'red') + xlab("Observed Total Mass in kg") + ylab("Predicted Total Fat Mass in kg") + ggtitle("Random Forest Prediction")
```
...
```{r}
result %>% ggplot(aes(x = as.numeric(marginal.beta), y = as.numeric(bi.beta))) + geom_point() + geom_abline(intercept = 0, slope = 1, col = 'red')
```
...
# Missed original signal

```{r}
sum(result$marginal.p[-which(result$bi.p<5e-8)]<5e-8)
```

# Manhatthan plot

```{r}
bim <- genio::read_bim("/Users/jianhuigao/Library/CloudStorage/OneDrive-UniversityofToronto/EHR_research/SyntheticSurrogateAnalysis/Data/final.bim") %>%
  rename(rsid = id)
result <- result %>% inner_join(bim)
